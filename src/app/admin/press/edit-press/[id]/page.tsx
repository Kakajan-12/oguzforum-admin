...
const EditPress = () => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/press-cat`);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/press/${id}`, {
                    headers: {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await axios.put(
                `${process.env.NEXT_PUBLIC_API_URL}/api/press/${id}`,
                formData,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            router.push(`/admin/press/view-press/${id}`);
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="flex bg-gray-200 min-h-screen">
            ...
            <div className="flex-1 p-10 ml-62">
                <TokenTimer />
                <div className="mt-8">
                    <h1 className="text-2xl font-bold mb-4">Edit Press</h1>
                        ...
                        <div className="mb-4 flex space-x-4">
                            ...
                            <div className="w-full">
                                ...
                                <select
                                    id="press_cat"
                                    name="press_cat"
                                    value={String(data.press_cat_id)}
                                    onChange={(e) => setData((prev) => ({ ...prev, press_cat_id: e.target.value }))}
                                    required
                                    className="border border-gray-300 rounded p-2 w-full focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-150"
                                >
