...
const ViewTypes = () => {
    const { id } = useParams();
    const [data, setData] = useState<Data | null>(null);  // Типизация состояния
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/types/${id}`, {
                    headers: {
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('auth_token');
            await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/types/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            ...
            <div className="flex-1 p-10 ml-62">
                ...
                <div className="mt-8">
                    <div className="w-full flex justify-between">
                        <h2 className="text-2xl font-bold mb-4">View Types</h2>
                                ...
                                <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                                    <div className="py-1">
                                        <Menu.Item>
                                            {({ active }) => (
                                                <button
                                                    onClick={() => router.push(`/admin/types/edit-type/${id}`)}
                                                    className={`${
                ...
                {showModal && (
                    <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 z-50">
                        <div className="bg-white p-6 rounded shadow-md w-96">
                            <h2 className="text-lg font-bold mb-4">Remove type</h2>
