...
const ViewPressCategory = () => {
    const { id } = useParams();
    const [data, setData] = useState<PressCategory | null>(null);
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('auth_token');
            await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/press-cat/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            ...
            <div className="flex-1 p-10 ml-62">
                ...
                <div className="mt-8">
                    <div className="w-full flex justify-between">
                        <h2 className="text-2xl font-bold mb-4">View Press Category</h2>
                                ...
                                <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                                    <div className="py-1">
                                        <Menu.Item>
                                            {({ active }) => (
                                                <button
                                                    onClick={() => router.push(`/admin/press-category/edit-press-category/${id}`)}
                                                    className={`${
                ...
                {showModal && (
                    <div className="fixed inset-0 flex items-center justify-center bg-opacity-50 z-50">
                        <div className="bg-white p-6 rounded shadow-md w-96">
                            <h2 className="text-lg font-bold mb-4">Remove press category</h2>
                            <p className="mb-6">Are you sure you want to delete this press category?</p>
...
export default ViewPressCategory;
